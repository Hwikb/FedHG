...
from flcore.clients.clienthg import clientHG
...
class FedHG(Server):
    def __init__(self, args, times):
        ...
        self.localize_feature_extractor = args.localize_feature_extractor  #局部特征提取器
        if self.localize_feature_extractor:
            self.global_model = copy.deepcopy(args.model.head)
        self.alphag=args.alphag
        self.betag=args.betag
        self.tground=args.tground
    ...
    def send_models(self,round):
        ...
        for client in self.clients:
            ...
            client.set_parametersH(self.generative_model)
            #client.set_parameters(self.global_model,self.generative_model)
            ####
            client.local_initialization(self.global_model,round,self.tground)
    ...
    def train_generator(self):
        ...
        for _ in range(self.server_epochs):
            ...
            z = torch.clamp(self.generative_model(labels), min=-10, max=10)
            soft_loss = nn.KLDivLoss(reduction="batchmean")
            #KL_loss1=torch.tensor(0.0,device=self.device)
            #KL_loss2=torch.tensor(0.0,device=self.device)
            a1,a2=self.alphag,self.betag
            label_matrix = labels.unsqueeze(1).expand(self.batch_size, self.batch_size)

            # Create a mask for identifying the same and different labels
            same_label_mask = (label_matrix == label_matrix.T).fill_diagonal_(False)
            diff_label_mask = (label_matrix != label_matrix.T)

            # Compute softmax for all z values once
            z_softmax = F.softmax(z, dim=1)
            z_log_softmax = F.log_softmax(z, dim=1)

            # KL loss for same label pairs
            KL_loss1 = torch.sum(
                F.kl_div(z_log_softmax.unsqueeze(1), z_softmax.unsqueeze(0), reduction='none')[same_label_mask])

            # KL loss for different label pairs
            KL_loss2 =torch.sum(
                F.kl_div(z_log_softmax.unsqueeze(1), z_softmax.unsqueeze(0), reduction='none')[diff_label_mask])
            ...
            for w, model in zip(self.uploaded_weights, self.uploaded_models):
                ...
                if self.localize_feature_extractor:
                    ...
                    logits += model.head(z) * w   #每个本地模型都出了一份力来得到这个logits

            self.generative_optimizer.zero_grad()
            H=a1*(1-1/(KL_loss1+1))+a2*(1/(KL_loss2+1))
            loss = self.loss(logits, labels)
            loss+=H

            loss.backward()
            #torch.nn.utils.clip_grad_norm_(self.generative_model.parameters(),max_norm=0.5)
